...
const parseResponse = ( response ) => { buildPromise( { sessionId: processSessionId( response ), captcha: generateImageBase64( response.body ) } ) }
const getParams = () => { getData().then( response => validateRequest( response, 'Impossível capturar o captcha!')).then( parseResponse ) }
